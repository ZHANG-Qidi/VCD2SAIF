;;; Version 1.2
;;; Date 2022年  1月 29日 土曜日 10:53:30 JST
;;; =====================================包定义区=====================================
...
(defun make_instance_output (instance node_info_ht value_changes_ht instance_list data_index)
  (let ((instance_output nil))
    (cond ((not (null (car instance)))
           ...
           (cond ((not (null (gethash instance_list node_info_ht)))
                  (let ((NET (list "NET"))
                        (identifier_index_list (gethash instance_list node_info_ht)))
                    (dolist (obj identifier_index_list)
                      ;; 从value_changes_ht 和 node_info_ht中抽取信息构建NET
                      (append1f NET (append (list (car obj)) (cadr (nth data_index (gethash (nth 2 obj) (gethash (nth 1 obj) value_changes_ht)))))))
                    (append1f instance_output NET))))
           ;; 对剩余的instance列表进行遍历，遍历因为可能存在同级别instance
           (dolist (obj (cdr instance) instance_output)
             (append1f instance_output (make_instance_output obj node_info_ht value_changes_ht instance_list data_index)))))))
;;; ===================================== SAIF函数结束 ==================================
;;; ===================================== 打印函数定义区 =====================================
;;; 输出列表格式化
    ...
    (pprint-logical-block (*standard-output* list :prefix "(" :suffix ")")
      (dolist (obj list)
        (cond ((listp obj)
               ...
               (cond ((or (and (equal "INSTANCE" (car obj))
                               (atom (cadr obj)))
...
(defun vcd2saif (vcd_file &key (simulation_time 0) (split_interval 0) (tcl_count 0))
  (let* ((*print-right-margin* 10000)
                     ...
                     (cond ((char= #\# (aref obj 0))
                                   ...
                                   (cond ((and (/= split_interval 0)
                                          ...
                                          (maphash #'(lambda (k v)
                                                       ...
                                                       (maphash #'(lambda (k1 v1)
                                                                    (declare (ignore k1))
                                                                    (let* ((time_value_old (nth 0 v1))
                                                                           (split_start (nth 1 v1))
                                                                           ;; 将旧信号信息赋值给 split_end
                                                                           (split_end (copy-tree time_value_old))
                                                                           (split_range (nth 2 v1))
                                                                           (simulation_time_old (nth 0 (nth 0 time_value_old)))
                                                                           (scalar_value_old (nth 1 (nth 0 time_value_old))))
                                                                      ;; 根据 split_time 更新 split_end
                                                                      (incf (cadr (nth (cdr (assoc scalar_value_old '((#\0 . 0) (#\1 . 1) (#\x . 2) (#\z . 3))))
                                                                                       (cadr split_end)))
                                                                            (- split_time simulation_time_old))
                                                                      (setf (caar split_end) split_time)
                                                                      ;; 计算 split_range
                                                                      ;; 判断是否为第一次处理，如果是则 split_start 还未被初始化
                                                                      (cond ((= split_time split_interval)
                                                                             ;; split_range 赋值为 split_end
                                                                             (setf (nth 0 split_range) (copy-tree (nth 0 split_end)))
                                                                             (setf (nth 1 split_range) (copy-tree (nth 1 split_end))))
                                                                            (t
                                                                             ;; split_range 赋值为 split_end 与 split_start 的差
                                                                             ;; split_range 信号和时间信息赋值为信号变化和时间变化
                                                                             (setf (car split_range) (list (list (caar split_start) (caar split_end))
                                                                                                           (list (cadar split_start) (cadar split_end))))
                                                                             (do ((i 0 (1+ i)))
                                                                                 ((= i 6) 'done)
                                                                               ;; 后续累计信息赋值为差值
                                                                               (setf (cadr (nth i (cadr split_range)))
                                                                                     (- (cadr (nth i (cadr split_end)))
                                                                                        (cadr (nth i (cadr split_start))))))))
                                                                      ;; 更新 value_changes_split_start_ht
                                                                      ;; 只能分割一次
                                                                      (cond ((< simulation_time_new (+ split_time split_interval))
                                                                             ;; 更新 split_start 为 split_end
                                                                             (setf (nth 0 split_start) (copy-tree (nth 0 split_end)))
                                                                             (setf (nth 1 split_start) (copy-tree (nth 1 split_end))))
                                                                            (t
                                                                             ;; 从上次信号变化到当前时间戳，时间区间满足多次分割
                                                                             ;; 将旧信号列表赋值给 split_start
                                                                             (setf (nth 0 split_start) (copy-tree (nth 0 time_value_old)))
                                                                             (setf (nth 1 split_start) (copy-tree (nth 1 time_value_old)))
                                                                             (let (;; 计算新的 split_time，为距离当前时间戳刚好不满足一个时间间隔
                                                                                   (split_time (- simulation_time_new (mod simulation_time_new split_interval))))
                                                                               ;; 更新 split_start 的信号累计时间
                                                                               (incf (cadr (nth (cdr (assoc scalar_value_old '((#\0 . 0) (#\1 . 1) (#\x . 2) (#\z . 3))))
                                                                                                (cadr split_start)))
                                                                                     (- split_time simulation_time_old))
                                                                               ;; 更新 split_start 的时间戳信息
                                                                               (setf (caar split_start) split_time))))))
                                                                v))
                                                   value_changes_ht)
                                          (setf (saif_struct-duration saifile) (list "DURATION" split_interval))
                                          (setf (saif_struct-instance_output saifile) (make_instance_output instance node_info_ht value_changes_ht instance_list 2))
                           ...
                           ((member (aref obj 0) '(#\0 #\1 #\x #\z))
                            (let* (;; 读取变量标识符
                                   ...
                                   (value_list (nth 0 (gethash 0 (gethash identifier_code value_changes_ht))))
                                   ;; 根据标识符从 value_changes_ht 中读取信号上次改变的时间戳
                                   (simulation_time_old (nth 0 (car value_list)))
                                   ;; 根据标识符从 value_changes_ht 中读取信号的当前为止的值
                                   (scalar_value_old (nth 1 (car value_list)))
                                   ;; 读取当前信号值
                                   (scalar_value_new (aref obj 0)))
                              ;; 更新 value_changes_ht 中的信号时间戳
                              (setf (nth 0 (car value_list)) simulation_time_new)
                              ;; 更新 value_changes_ht 中的变量值信息
                              (setf (nth 1 (car value_list)) scalar_value_new)
                              ;; 如果时间信息不为0
                              (cond ((/= 0 simulation_time_new)
                                     ...
                                     (cond ((or (and (char= #\0 scalar_value_old)
                                            ...
                                            (incf (cadr (nth 5 (cadr value_list))))))))))
                           ;; 当单词以b字符起始时，为多变量变化信息
                           ((char= #\b (aref obj 0))
                            (let* (;; 从下一个单词读取变量标识符
                              ...
                              (let ((value_length (length vector_value)))
                                ...
                                ;; 根据多位变量的 size 进行补齐
                                (let ((value_high (cdr (assoc (aref vector_value 0) '((#\0 . #\0) (#\1 . #\0) (#\x . #\x) (#\z . #\z))))))
                                  (do ((i value_length (1+ i)))
                                      ((= i size) 'done)
                                    ;; 根据最高位决定具体赋值，延伸最高位
                                    (push value_high vector_value_list))))
                              ;; 反转 vector_value_list 变成从低到高
                              (setf vector_value_list (reverse vector_value_list))
                              (do ((i 0 (1+ i)))
                                  ((= i size) 'done)
                                ;; 更新value_changes_ht 中对应变量的所有信息
                                (let* ((value_list (nth 0 (gethash i (gethash identifier_code value_changes_ht))))
                                       (simulation_time_old (nth 0 (car value_list)))
                                       (scalar_value_old (nth 1 (car value_list)))
                                  ...
                                  (t
                                   (let (;; 读取变量标识符和变量名
                                     ...
                                     (cond (;; 确定当前变量为单变量
                                            ...
                                            ;; 2. 分割起始状态
                                            ;; 3. 分割区间状态
                                            (setf (gethash 0 (gethash identifier_code value_changes_ht)) (list (list (list 0 "T?") (list (list "T0" 0) (list "T1" 0) (list "TX" 0) (list "TZ" 0) (list "TB" 0) (list "TC" 0)))
                                                                                                               (list (list 0 "T?") (list (list "T0" 0) (list "T1" 0) (list "TX" 0) (list "TZ" 0) (list "TB" 0) (list "TC" 0)))
                                                                                                               (list (list 0 "T?") (list (list "T0" 0) (list "T1" 0) (list "TX" 0) (list "TZ" 0) (list "TB" 0) (list "TC" 0)))))
                                            ;; 更新lines
                                            (setf lines (cdr (cddddr lines))))
                                           (t
                                            ...
                                            (let ((start 0)
                                              ...
                                              (do ((i start (1+ i)))
                                                ...
                                                (setf (gethash (- i start) (gethash identifier_code value_changes_ht)) (list (list (list 0 "T?") (list (list "T0" 0) (list "T1" 0) (list "TX" 0) (list "TZ" 0) (list "TB" 0) (list "TC" 0)))
                                                                                                                             (list (list 0 "T?") (list (list "T0" 0) (list "T1" 0) (list "TX" 0) (list "TZ" 0) (list "TB" 0) (list "TC" 0)))
                                                                                                                             (list (list 0 "T?") (list (list "T0" 0) (list "T1" 0) (list "TX" 0) (list "TZ" 0) (list "TB" 0) (list "TC" 0)))))))))))))
                           ((equal "$upscope" obj)
                           ...
                           ((equal "dumpvars" obj))
                           ((equal "dumpoff" obj))))))))
    ;; vcd文件保存的最终时间小于设定的终止时间，重新设定模拟时间到设置的终止时间
                 ...
                 (maphash #'(lambda (k1 v1)
                              ...
                              (let* ((time_value_old (nth 0 v1))
                                ...
                                (incf (cadr (nth (cdr (assoc scalar_value_old '((#\0 . 0) (#\1 . 1) (#\x . 2) (#\z . 3))))
                                                 (cadr time_value_old)))
                                      (- simulation_time_new simulation_time_old))
                                (setf (caar time_value_old) simulation_time_new)
                                ;; 如果分割未完成，更新 split_range
                                (let (;; 将旧信号信息赋值给 split_end
                                      (split_end (copy-tree time_value_old)))
    ...
    (setf (saif_struct-instance_output saifile) (make_instance_output instance node_info_ht value_changes_ht instance_list 0))
    (let ((saif_file (concatenate 'string
                                  (subseq vcd_file 0 (position #\. vcd_file :from-end t))
                                  "_vcd2saif.saif")))
